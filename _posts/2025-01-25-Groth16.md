...
{: .warning}
It is useful to recall our notation for [group elements](#pairing-friendly-groups) and for [Lagrange polynomials](#notation-and-background) $\lagr_i(X)$.
...
In this blog post, I'll describe (what I think is) a simpler method based on [polynomial differentiation](/differentiation-tricks#interpolating-fxgx-in-lagrange-basis), which is used quite often in the literature[^TCZplus20]$^,$[^TABplus20]$^,$[^Habo22e].
...
Fortunately, we can apply some [differentiation tricks](/differentiation-tricks#interpolating-fxgx-in-lagrange-basis):

{: .theorem}
$\forall f,g,h\in\F[X]$ such that $f(X) = g(X)h(X)$, if $g(\omega^i) = 0$, then we have $f'(\omega^i) = g'(\omega^i) h(\omega^i)$.

(We give a proof [here](/differentiation-tricks).)
...
In a real-world implementation, the equation above is slightly-reorganized so as to compute the 3 pairings above faster via a [multi-pairing](/pairings#multi-pairings):
\begin{align}
...
In the appendix, we describe a [$\Gr_T$ MSM reformulation of $\grothVerify$](#groth16-verify-gt-msm) which also applies here, except with preprocessed $\crs{T_j} \bydef \three{\uvw}$, yielding:
\begin{align}
\label{eq:bgm17-verify-gt}
\alphabeta + \pair{\one{A}}{\two{B}} + \sum_{j=0}^\ell a_j \crs{T_j} + \pair{\one{C}}{\deltaTwo} \equals \three{0}
\end{align}
{: .smallnote}
...
This is very useful in privacy-preserving settings, where the verifier may only have a commitment to $\stmt$ instead of the statement itself (e.g., see zk-creds[^RWGM22e] or the [keyless relation](#appendix-keyless-blockchain-accounts-as-an-example) in Eq. \ref{eq:keyless-rel}).
...
Given a [BGM17]-style proof $\pi$ for $\stmt$, this algorithm returns a Pedersen commitment $c$ to $\stmt$ and a new re-randomized proof $\pi'$ that verifies against $c$.

First, picks random blinding factors $(\Delta{r}, \Delta{s})\randget \F^2$ and re-randomizes the proof $\pi$ via $\grothRerand$ (in place):
\begin{align}
    (\one{A'},\two{B'},\one{C'}) \gets \grothRerand(\vk, \pi; \Delta{r},\Delta{s})
\end{align}
...
(This can be done via a $\Sigma$-protocol; e.g., for $\ell=1$ see the [ZKPoK protocol in DeKART](/dekart#preliminary-zkpoks).)
...
Your thoughts or comments are welcome on [this thread](https://x.com/alinush/status/1883922994973860292).

## Appendix: Replacing $\grothVerify$'s $\Gr_1$ MSM with a $\Gr_T$ MSM
{: #groth16-verify-gt-msm}

In some settings, such as when wrapping one or more _inner_ Groth16 proof(s) using an _outer_ zkSNARK scheme, it can be useful to replace the $\Gr_1$ MSM in the Groth16 verifier with $\Gr_T$ one.
This eliminates all elliptic curve operations from the Groth16 verifier, potentially making it faster when expressed as a constraint system for the outer zkSNARK.
(Naturally, it does make it slower if verified on a CPU rather than inside a constraint system.)

First, pull the sum outside the 2nd pairing from Eq. \ref{eq:groth16-verify}:
\begin{align}
    \pair{\sum_{j=0}^\ell a_j \uvwGammaOne}{\gammaTwo}
    &= \sum_{j=0}^\ell a_j \cdot \pair{\uvwGammaOne}{\gammaTwo}
\end{align}

Define the **preprocessed** $\Gr_T$ elements:
\begin{align}
    \crs{T_j} \bydef \pair{\uvwGammaOne}{\gammaTwo}
\end{align}

These depend only on the circuit (not the proof or public inputs) and can be computed once during VK post-processing.
The verification from Eq. \ref{eq:groth16-verify} becomes:
\begin{align}
\label{eq:groth16-verify-gt}
\alphabeta + \pair{\one{A}}{-\two{B}} + \sum_{j=0}^\ell a_j \crs{T_j} + \pair{\one{C}}{\deltaTwo} \equals \three{0}
\end{align}
...
[^ultra-groth]: [UltraGroth Protocol Explained](https://hackmd.io/@ZamDimon/SkFGEXbKgx), ZamDimon, 2025
