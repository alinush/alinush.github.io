...
We assume familiarity with the [Spartan protocol](/spartan) and its mathematical preliminaries:
 ...
 - [Lagrange polynomials for interpolating MLEs](/mle#lagrange-polynomials)
...
The original Spartan paper[^Sett19e] gives a dense-to-sparse compiler based on memory checking and/or GKR (it's rather difficult to parse).

{: .todo}
Setty and Lee[^SL20] later give a different compiler.
Other works that give dense-to-sparse compilers are Lasso[^STW23e], LogUp[^Habo22e], LogUp$^\*$[^Souk25e] 
Bunz et al give a permutation argument that could be converted into a lookup argument and yield a dense-to-sparse compiler Spartan[^BCD25e].
...
\end{align}
(These can be interpolated [as usual](/spartan#multilinear-extensions-mles).)
...
#### Verifier time
 ...
 1. Derive dense MLE PCS commitment for random linear combination polynomial $P(\Z)$
...
Big thanks to [Yi-Nuo Zhang](https://scholar.google.com/citations?user=LIaZUmsAAAAJ&hl=en) for the prototype implementation of Cinder that was used in our [benchmarks](#benchmarks).
