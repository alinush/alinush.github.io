...
We assume familiarity with:
 ...
 - [Baby-step giant-step (BSGS)](/bsgs) discrete log algorithms
...
First, it allows us to use the [naive DL algorithm](/bsgs#naive-discrete-log-algorithm) to instantly decrypt TXN amounts.
...
The **main tension** is between:
 ...
 2. The decryption time for a TXN's amount: i.e., the chunk size $b$
    ...
    - We must be able to compute DLs on $(b+t)$-bit encrypted balance chunks, **reasonably** fast, in the browser
        + DL times are fast for 32 bits (see WASM benchmarks [here](/bsgs#wasm-dlp-tbsgs-k-and-naivetruncateddoubledlookup)) $\Rightarrow b = t = 16$
